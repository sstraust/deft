...
(defmacro witht [def-list & code]
  (let [[class-name var-name  & {:keys [allow-overrides skip-fields]}] def-list
    ...
    (println (:name (api/resolve &env class-name)))
    (doseq [class-field (get @defc-fields-map
                             (symbol (:name (api/resolve &env class-name))))]
      (let [var-name (symbol (str (namespace (symbol (:name (api/resolve &env class-name)))))
                             (str class-field))]
...
"\n(witht [YourClass your-var :allow-override [your-field-name]]
   ....
    ...
    `(let [{~(keyword (namespace (symbol (:name (api/resolve &env class-name)))) (name :keys))
            ~(into [] (remove
